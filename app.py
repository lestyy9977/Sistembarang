...
# Edit Barang
@app.route('/barang/edit/<uid>', methods=['GET', 'POST'])
def edit_barang(uid):
    if request.method == 'POST':
        # tangkap dulu data dari form
        data = {
        ...
        # simpan ke database
        # cara tambah data dari backend ke firestore
        db.collection('barang').document(uid).update(data) #set(data, merge=True)
        flash('berhasil edit barang','success')
    # memanggil data berdasarkan ID
    data = db.collection('barang').document(uid).get().to_dict()
    return render_template('barang/edit.html', data=data)
...
# Hapus Kategori
@app.route('/kategori/hapus/<uid>')
def hapus_kategori(uid):
    db.collection('kategori').document(uid).delete()
    flash('Berhasil Hapus Kategori', 'danger')
